...
namespace ProjectInfinity.Content.CustomHooks
{
	class MovingPlatforms : HookGroup
	{
		...
		private void PlatformCollision(On.Terraria.Player.orig_SlopingCollision orig, Player self, bool fallThrough, bool ignorePlats)
		{
			...
			{
                if (!NPC.active || NPC.ModNPC == null || !(NPC.ModNPC is PlatformBoss))
                    continue;
				if((NPC.ModNPC as PlatformBoss).bossPlatformTimer > 0)
				{
                    orig(self, fallThrough, ignorePlats);
					(NPC.ModNPC as PlatformBoss).bossPlatformTimer = -1;
                    return;
                }
            }
