...
/*namespace ProjectInfinity.Core.Loaders
{
	class ShaderLoader : IOrderedLoadable
	{
		...
		public void Load()
		{
			...
			var file = (TmodFile)info.Invoke(ProjectInfinity.Instance, null);
		...
		{
			var screenRef = new Ref<Effect>(ProjectInfinity.Instance.Assets.Request<Effect>(path, ReLogic.Content.AssetRequestMode.ImmediateLoad).Value);
